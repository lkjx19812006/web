...
<template>
  <div class="order_confirm" :v-loading.body="findLoading">
        ...
        <div class="detail">
          <div class="address">
            ...
            <div class="address_item">
              <div :class="['address_content',item.selectShow ? 'addressActiveClass' : '']" v-for="item in addressList"
                   v-on:mouseenter="show(item)" v-on:mouseleave="hide(item)" @click="select(item)">
                <div class="name">
                  <div><span>{{item.contactName}}</span><span class="left_dis">{{item.contactPhone}}</span></div>
                  <div v-show="item.operateShow"><i class="el-icon-edit" v-on:click.stop="edit(item)"></i><i
                    class="el-icon-delete" v-on:click.stop="del(item)"></i></div>
                </div>
            ...
            <div class="address_item need_table">
              <div class="table">
                <el-table :data="cart" style="width: 100%;border:none;text-align:center">
                    ...
                    <template scope="scope">
                      <div class="goods">
                        ...
                        <div class="goods_information">
                          ...
                          <div><span>规格:{{scope.row.spec}}</span><span class="left_dis">产地:{{scope.row.location}}</span>
                          </div>
                        </div>
                      </div>
                    </template>
                  ...
                  <el-table-column prop="buyNumber" label="数量" width="200">
                    <template scope="scope">
                      <span>{{scope.row.buyNumber + scope.row.unit}}</span>
                    </template>
                  </el-table-column>
                  <el-table-column label="小计(元)" width="300">
                    <template scope="scope">
                      <span>￥{{scope.row.price, scope.row.buyNumber | filterPrice}}</span>
                    </template>
                  </el-table-column>
                </el-table>
              </div>
...
<script>
  ...
  export default {
      ...
      getAddress() {
        ...
        common.commonPost(url, body).then((res) => {
          res.biz_result.list.forEach(item => {
            ...
            if (item.type === 1 || res.biz_result.list.length === 1) {
    ...
    mounted() {
      if (window.localStorage.cart) this.cart.push(JSON.parse(window.localStorage.cart));
      else {
        return this.$router.push("/resource");
      }
      console.log(this.cart[0].buyNumber)
      if (!this.cart[0].buyNumber) {
        return this.$router.push("/resource");
      }
      this.number = this.cart[0].buyNumber;
