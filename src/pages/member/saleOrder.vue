...
<template>
  <div class="sale_order">
    ...
    <div class="status">
      <div v-for="(item,index) in sellStatus" v-bind:class="{color:item.show}" @click="checkStatus(item.back_id,index)">
        {{item.name}}
      </div>
    </div>
    <el-table :data="orderList" style="width: 100%">
      ...
      <el-table-column label="操作">
        <template scope="scope">
          ...
          <div class="seelogis" v-if="'send' ==judgeOrderStatus(orderList[scope.$index].orderStatus)"
               @click="seeLogistics(scope.$index, scope.row)">查看物流
          </div>
          <div class="seelogis" v-if="'waitsend' == judgeOrderStatus(orderList[scope.$index].orderStatus)"
               @click='delivery(orderList[scope.$index].id,orderList[scope.$index].no)'>我要发货
          </div>
    ...
    <el-dialog v-model="edit" size="tiny" title="物流信息">
      <logisticsView :param="logistics"></logisticsView>
    </el-dialog>
...
<script>
  ...
  import logisticsView from '../../components/common/logistics'
    ...
    methods: {
      ...
      seeLogistics(index, row) {
        ...
        common.commonPost(url, body)
          .then(function (res) {
            if (res.biz_result.data) {
              _self.logistics.arr = res.biz_result.data;
              _self.logistics.show = true;
            } else {
              _self.logistics.arr = [];
              _self.logistics.show = false;
            }
            _self.loading = false;
          })
          .catch(function (err) {
