...
<template>
    <div class="head_content">
        ...
        <div class="head_welcome" ref="header">
            <div class="welcome_center">
                <div class="welcome_left">
                    ...
                    <span v-show="!user.phone"><span class="dis_span">【</span><span class="orange_span" @click="jump('/login')">用户登录</span>|<span class="orange_span" @click="jump('/register')">免费注册</span><span>】</span></span>
                    <span v-show="user.phone && !user.fullname" class="orange_span">{{user.phone}}</span>
                    <span v-show="user.fullname" class="orange_span">{{user.fullname}}</span>
        ...
        <div class="left_static" :style="{ marginTop: disTop + 'px' }" v-show="showNav">
            <div class="district" :class="{'active':item.name === pathName && !hoverShow}" v-for="(item,index) in leftArray" @click="searchId(index)" v-on:mouseenter="show(index)" v-on:mouseleave="hide(index)">
                <img :src="item.image">
                <p>{{item.title}}</p>
            </div>
