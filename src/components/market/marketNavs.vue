...
<template>
  <div>
    <div class="marketnav" v-for="title in titles">
      ...
      <div class="select">
        <div class="item" v-bind:class="{active:item.show && !isSearch}" v-for="(item,index) in title.titleList"
             :key="index" @click="selectClick(item,title)">{{item.name}}
        </div>
      </div>
    </div>
    <div class="search">
      ...
      <div class="search_content">
        <search v-model="breedName" style="width: 100%" placeholder="请输入您需要的产品名称"
  ...
  export default {
    ...
    methods: {
      ...
      handleSelect(val) {
        ...
        this.httpParam.categoryId = '';
        this.httpParam.name = this.breedName;
        this.httpParam.breedId = val.breedId;
        this.$emit("result", this.httpParam);
      },
