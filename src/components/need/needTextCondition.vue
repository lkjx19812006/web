...
<template>
  <div class="need_text_condition">
    <div class="condition_detail">
      ...
      <mySearch v-model="httpParam.keyWord" placeholder="请输入查询的药材名称" v-on:fetch-suggestions="querySearchAsync"
                v-on:select="handleSelect"></mySearch>
    ...
    <div class="condition_detail">
      <p class="condition_detail_label">{{type === 'need' ? '交货地' : '产地'}}</p>
      <!--type === need-->
      <el-select v-if="type === 'need'" v-model="httpParam.province" filterable multiple
                 placeholder="请选择" :change="selectValue()">
        <el-option-group v-for="group in options" :label="group.key" :key="group">
          <el-option v-for="(item, index) in group.value" :key="index" :label="item.name" :value="item.id">
          </el-option>
        </el-option-group>
      </el-select>
      <!--type === 其他-->
      <el-select v-else v-model="httpParam.location" filterable multiple
                 placeholder="请选择" :change="selectValue()">
        <el-option-group v-for="group in options" :label="group.key" :key="group">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getLocation() {
        let _self = this;
        let module = 'breedService';
        let method = 'queryBreedLocalList';
        if (this.type === 'need') {
          module = 'locationService';
          method = 'queryProvinceByChina';
        }
