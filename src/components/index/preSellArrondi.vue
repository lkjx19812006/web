            ...
            <div v-for="(subItem,index) in item" :key="index" :class="[index%3==1 ? 'marginClass' : '', 'good_detail']">
              <div class="content">
                <div class="img_content">
                  <img class="big_img" style="cursor: pointer" :src="subItem.image[0]" v-on:click="offer(subItem.id)"
                       v-if='subItem.restNumber > 0'>
                ...
                <div class="name" style="cursor: pointer" v-if='subItem.restNumber > 0' v-on:click="offer(subItem.id)">
                  {{subItem.breedName}}
                </div>
                <div class="name" style="cursor: default" v-if='subItem.restNumber <= 0'>{{subItem.breedName}}</div>
                <div class="detail">
                  <div class='left'>规格：{{subItem.spec}}</div>
                  <div class='right'>剩余数量：<span
                    style="color: #F98435; font-size: 18px;">{{subItem.restNumber}}{{subItem.unit}}</span></div>
                </div>
                <div class="detail">
              ...
              <div class="detail_button">
                ...
                <el-button size="small" class="green_button" type="primary" v-on:click="offer(subItem.id)"
                           v-if='subItem.restNumber > 0'>立即抢购
                </el-button>
                <el-button size="small" class="green_button" style="cursor: default" type="primary"
                           v-if='subItem.restNumber <= 0'>立即抢购
                </el-button>
              </div>
            </div>
